...
@section('container')
    ...
    <div class="px-3 py-6 md:p-6 mb-5">
        <h1 class="font-semibold text-2xl md:text-3xl mb-3">Data Absen Kegiatan Wajib</h1>
        <div class="text-gray-600 text-sm">Daftar seluruh data absen kegiatan wajib mahasiswa Asrama Polbangtan-mlg</div>
        ...
        <div class="bg-white rounded-lg p-3 mt-5 border-2">
            <div class="flex flex-col sm:flex-row items-center sm:item-end gap-2">
                <div class="w-full sm:w-[300px]">
                    {{-- <form action="{{ route('admin.dataKegiatanWajibSearch') }}" method="post">
                        @csrf --}}
                    <div class="relative">
                        ...
                        <input type="search" id="searchInput" name="searchInput" oninput="performSearch(this.value)"
            ...
            @if (count($Users) > 0)
                <div class="relative overflow-x-auto border-2 border-gray-200 sm:rounded-lg mt-2">
                    <table class="w-full text-sm text-left">
                        ...
                        <tbody id="result">
                            @include('admin.partials.data_kegiatan_table', [
                                'Users' => $Users,
                            ])
                        </tbody>
                    </table>
    ...
    <meta name="csrf-token" content="{{ csrf_token() }}">
    <script src="{{ asset('js/dataKegiatan.js') }}" type="text/javascript"></script>
@endsection
