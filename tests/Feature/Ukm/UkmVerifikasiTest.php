...
class UkmVerifikasiTest extends TestCase
{
    ...
    public function test_pelatih_ukm_ditolak_mengakses_verifikasi(): void
    {
        $pelatih = $this->makeUser(User::PELATIH_UKM_ROLE_ID);
        $ukm = Ukm::create(['nama' => 'UKM Tolak', 'slug' => 'ukm-tolak']);
        
        UkmMember::create(['ukm_id' => $ukm->id, 'user_id' => $pelatih->id, 'peran' => 'pelatih', 'status' => 'aktif']);

        $response = $this->actingAs($pelatih)->get(route('admin.ukm.verifikasi.index', $ukm->id));
    ...
    public function test_admin_ditolak_mengakses_verifikasi(): void
    {
        $admin = $this->makeUser(User::ADMIN_ROLE_ID);
        $ukm = Ukm::create(['nama' => 'UKM Futsal', 'slug' => 'ukm-futsal']);

        $response = $this->actingAs($admin)->get(route('admin.ukm.verifikasi.index', $ukm->id));
        $response->assertStatus(302);
    }
}
